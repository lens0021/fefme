...
export default class UserDataPoller {
	...
	start(): void {
		if (this.intervalRunner) {
			this.logger.trace(
				`User data poller already running, not starting another one`,
			);
			return;
		}

		this.logger.info(
			`Starting UserDataPoller on ${config.api.backgroundLoadIntervalMinutes} minute interval...`,
		);
		...
		this.intervalRunner = setInterval(
			async () => {
				...
				if (!shouldContinue) {
					this.logger.info(
						`Finishing up data poller (shouldContinue=${shouldContinue})`,
					);
	...
	async getMoarData(): Promise<boolean> {
		...
		try {
			...
			const cacheSizes = await Promise.all(
				pollers.map(async (poll) => (await poll())?.length || 0),
			);
