 ...
 */
interface FetchParamsWithCacheData<T extends ApiObj>
	extends FetchParamsComplete<T>,
		MinMaxIDParams {
...
// Logging
const PARAMS_TO_NOT_LOG: FetchParamName[] = [
	"breakIf",
	"fetchGenerator",
	"logger",
	"processFxn",
];
const PARAMS_TO_NOT_LOG_IF_FALSE: FetchParamName[] = [
	"skipCache",
	"skipMutex",
	"moar",
];
...
export default class MastoApi {
	...
	private requestSemphore = new Semaphore(
		config.api.maxConcurrentHashtagRequests,
	); // Concurrency of search & hashtag requests
	...
	static async init(api: mastodon.rest.Client, user: Account): Promise<void> {
		if (!user.webfingerURI?.includes("@")) {
			apiLogger.logAndThrowError(
				`MastoApi.init() 'user' argument isn't an Account!`,
				user,
			);
	 ...
	 */
	apiErrorMsgs(): string[] {
		return this.apiErrors.map(
			(e) => e.message + (e.cause ? ` (cause: "${e.cause}")` : ""),
		);
	}
	...
	async fetchHomeFeed(params: HomeTimelineParams): Promise<Toot[]> {
		...
		if (moar) {
			...
			cutoffAt = maxTootedAt
				...
				: timelineCutoffAt();
			logger.debug(
				`maxTootedAt: ${quotedISOFmt(maxTootedAt)}, maxId: ${maxId}, cutoffAt: ${quotedISOFmt(cutoffAt)}`,
			);
		}
	...
	async getBlockedAccounts(): Promise<Account[]> {
		const blockedAccounts =
			(await this.getApiObjsAndUpdate<mastodon.v1.Account>({
	...
	async getCacheableToots(
		...
		try {
			...
			await this.handleApiError(
				{ cacheKey: cacheKey as CacheKey, logger },
				[],
				err,
			);
			return [];
	...
	async getFollowedAccounts(params?: ApiParams): Promise<Account[]> {
		return (await this.getWithBackgroundFetch<mastodon.v1.Account>({
			...
			fetchGenerator: () =>
				this.api.v1.accounts.$select(this.user.id).following.list,
	...
	async getFollowers(params?: ApiParams): Promise<Account[]> {
		return (await this.getWithBackgroundFetch<mastodon.v1.Account>({
			...
			fetchGenerator: () =>
				this.api.v1.accounts.$select(this.user.id).followers.list,
	...
	async getServerSideFilters(): Promise<mastodon.v2.Filter[]> {
		...
		const releaseMutex = await lockExecution(
			this.apiMutexes[CacheKey.SERVER_SIDE_FILTERS],
			logger,
		);
		const startTime = new Date();

		try {
			let filters = await Storage.getIfNotStale<mastodon.v2.Filter[]>(
			...
			if (!filters) {
				...
				logger.log(
					`Retrieved ${filters.length} filters ${ageString(startTime)}:`,
					filters,
				);
			...
			this.recordApiError(
				`Failed to fetch user's server-side filters`,
				err,
				logger,
			);
			return [];
	...
	async getUserData(force?: boolean): Promise<UserData> {
		...
		try {
			const hasNewest: boolean = this.userData
				? await this.userData.hasNewestApiData()
				: false;
	...
	async instanceInfo(): Promise<mastodon.v2.Instance> {
		let instanceInfo = await Storage.getIfNotStale<mastodon.v2.Instance>(
		...
		if (!instanceInfo) {
			try {
				...
				this.logger.error(
					`<instanceInfo()> Failed to fetch user's instance info, trying V1 API:`,
					err,
				);
				...
				if (v1Instance) {
					...
					this.logger.logAndThrowError(
						`Failed to fetch Mastodon instance info from both V1 and V2 APIs`,
						err,
					);
				}
			}
	...
	async isGoToSocialUser(): Promise<boolean> {
		if (isNil(this.isHomeserverGoToSocial)) {
			...
			if (typeof this.isHomeserverGoToSocial !== "boolean") {
				this.logger.warn(
					`Failed to set isHomeserverGoToSocial from sourceUrl "${instance?.sourceUrl}", instance`,
					instance,
				);
			} else if (this.isHomeserverGoToSocial) {
				this.logger.debug(
					`GoToSocial server detected based on instance:`,
					instance,
				);
			} else {
				this.logger.trace(
					`Not a GoToSocial server based on instance:`,
					instance,
				);
			}
	...
	async lockAllMutexes(): Promise<ConcurrencyLockRelease[]> {
		const allMutexes = Object.values(this.apiMutexes).concat(
			Object.values(this.cacheMutexes),
		);
		const mutexLogger = apiLogger.tempLogger("lockAllMutexes");
		mutexLogger.log(`Locking all mutexes...`);
		return await Promise.all(
			allMutexes.map((mutex) => lockExecution(mutex, mutexLogger)),
		);
	}
	...
	async searchForToots(
		...
		maxRecords = maxRecords || config.api.defaultRecordsPerPage;
		const releaseSemaphore = await lockExecution(this.requestSemphore, logger);
		const query: mastodon.rest.v1.SearchParams = {
			limit: maxRecords,
			q: searchStr,
			type: TrendingType.STATUSES,
		};
		const startedAt = new Date();
	...
	setSemaphoreConcurrency(concurrency: number): void {
		this.logger.trace(
			`Setting semaphore to background concurrency to ${concurrency}`,
		);
	...
	accountUrl(account: Account): string {
		return account.homeserver == this.homeDomain
			? account.url
			: this.endpointURL(`@${account.webfingerURI}`);
	}
	...
	private endpointURL = (endpoint: string): string =>
		`https://${this.homeDomain}/${endpoint}`;
	 ...
	 */
	private supportsMinMaxId = (cacheKey: CacheKey): boolean =>
		!!config.api.data[cacheKey]?.supportsMinMaxId;
	...
	private async fetchApiObjs<T extends ApiObj>(
		...
		try {
			for await (const page of fetchGenerator()(this.buildParams(params))) {
				...
				const resultsMsg =
					`got page ${++pageNumber} with ${page.length} objs ${waitTime.ageString()}` +
				...
				if (newRows.length >= maxRecords || page.length == 0 || shouldStop) {
					...
					logger.logAndThrowError(
						`Request took too long! (${waitTime.ageInSeconds()}s), ${resultsMsg}`,
					);
				...
				pageNumber % 5 == 0
					? logger.debug(resultsMsg)
					: logger.trace(resultsMsg);

				if (isBackgroundFetch) {
					const sleepMS =
						config.api.backgroundLoadSleepBetweenRequestsMS * Math.random(); // Jitter spaces out requests
					logger.trace(
						`Background fetch, sleeping for ${(sleepMS / 1000).toFixed(3)}s`,
					);
					await sleep(sleepMS);
				}
	...
	private async getApiObjsAndUpdate<T extends ApiObj>(
		...
		const releaseMutex = skipMutex
			? null
			: await lockExecution(this.cacheMutexes[cacheKey], hereLogger);
			...
			if (cacheResult && !moar) {
				if (cacheResult.isStale) {
					...
					if (this.apiMutexes[cacheKey].isLocked()) {
						hereLogger.trace(
							`Stale cache but update already in progress, returning stale rows`,
						);
					} else {
						hereLogger.debug(
							`Returning ${cacheResult.rows.length} stale rows and triggering cache update`,
						);
	...
	private async getApiObjs<T extends ApiObj>(
		...
		const releaseMutex = skipMutex
			? null
			: await lockExecution(this.apiMutexes[cacheKey], logger);
		...
		try {
			...
			newRows = UNIQUE_ID_PROPERTIES[cacheKey]
				? [...newRows, ...cachedRows]
				: newRows;
			const objs = this.buildFromApiObjects<T>(
				cacheKey,
				newRows as T[],
				logger,
			);
			...
			if (maxCacheRecords && objs.length > maxCacheRecords) {
				logger.warn(
					`Truncating ${objs.length} rows to maxCacheRecords=${maxCacheRecords}`,
				);
				...
				const sortedByCreatedAt = sortObjsByCreatedAt(
					objs as WithCreatedAt[],
				) as T[];
			...
			this.recordApiError(
				`Error fetching ${cacheKey} API data where there really shouldn't be!`,
				err,
				logger,
			);
			return [];
	...
	private async getWithBackgroundFetch<T extends ApiObj>(
		...
		const logger = this.loggerForParams(params).tempLogger(
			"getWithBackgroundFetch",
		);
		...
		if (objs.length < minRecords) {
			logger.log(
				`Fewer rows (${objs.length}) than required (${minRecords}), launching bg job to get the rest`,
			);
			this.getApiObjsAndUpdate<T>({
				...FULL_HISTORY_PARAMS,
				...params,
			...
			logger.trace(
				`Have enough rows (have ${objs.length}, want ${minRecords}), doing nothing`,
			);
		}
	 ...
	 */
	private buildParams<T extends ApiObj>(
		params: FetchParamsWithCacheData<T>,
	): PaginationParams {
	...
	private async addCacheDataToParams<T extends ApiObj>(
		...
		const minMaxIdParams: MinMaxIDParams = {
			maxIdForFetch: null,
			minIdForFetch: null,
		};
		...
		if (cacheResult?.minMaxId) {
			if (moar) {
				if (maxId)
					logger.warn(
						`maxId param "${maxId}" will overload minID in cache "${cacheResult.minMaxId.min}"!`,
					);
	...
	private async getCacheResult<T extends ApiObj>(
		...
		return {
			...
			minMaxId: this.supportsMinMaxId(cacheKey)
				? findMinMaxId(rows as ApiObjWithID[])
				: null,
			rows,
	...
	private async handleApiError<T extends ApiObj>(
		...
		logger = logger
			? logger.tempLogger("handleApiError")
			: getLogger(cacheKey, "handleApiError");
		logger.trace(`Handling API error for params:`, params, `\nerror:`, err);
		const cachedRows = cacheResult?.rows || [];

		if (
			!newRows?.length &&
			requestDefaults.canBeDisabledOnGoToSocial &&
			(await this.isGoToSocialUser())
		) {
			const goToSocialWarning =
				config.api.errorMsgs.goToSocialHashtagTimeline(cacheKey);
			this.recordApiError(
				logger.line(`Failed to fetch data. ${goToSocialWarning}`),
				err,
				logger,
			);
			return cachedRows;
		...
		if (UNIQUE_ID_PROPERTIES[cacheKey]) {
			logger.warn(
				`${msg} Merging cached + new rows on uniq property: "${UNIQUE_ID_PROPERTIES[cacheKey]}"`,
			);
			...
			if (newRows.length < cachedRows.length) {
				logger.warn(
					`${msg} Discarding new rows and returning old ones bc there's more of them.`,
				);
				return cachedRows;
			} else {
				logger.warn(
					`${msg} Keeping the new rows, discarding cached rows bc there's fewer of them.`,
				);
				return rows;
			}
		} else {
			logger.error(
				`Shouldn't be here! All endpoints either support min/max ID or unique IDs: ${msg}`,
			);
			return rows;
		}
	}
	...
	private buildFromApiObjects<T extends ApiObj>(
		...
		if (nullObjs.length) {
			logger.warn(
				`buildFromApiObjects() found ${nullObjs.length} null objects`,
				nullObjs,
			);
		}
		...
		if (STORAGE_KEYS_WITH_TOOTS.includes(key)) {
			...
			newObjects = objects.map((obj) =>
				Account.build(obj as AccountLike),
			) as ResponseRow<T>[];
		} else {
	 ...
	 */
	private fillInDefaultParams<T extends ApiObj>(
		params: FetchParams<T>,
	): FetchParamsComplete<T> {
		const { cacheKey, logger, maxRecords } = params;
		const requestDefaults = config.api.data[cacheKey];
		const maxApiRecords =
			maxRecords ||
			requestDefaults?.initialMaxRecords ||
			MIN_RECORDS_FOR_FEATURE_SCORING;

		const withDefaults: FetchParamsComplete<T> = {
			...params,
			limit: Math.min(
				maxApiRecords,
				requestDefaults?.limit ?? config.api.defaultRecordsPerPage,
			),
	...
	private loggerForParams<T extends ApiObj>(params: LogParams<T>): Logger {
		...
		return getLogger(
			cacheKey,
			moar && "moar",
			isBackgroundFetch && "backgroundFetch",
		);
	}
	 ...
	 */
	private shouldReturnCachedRows<T extends ApiObj>(
		params: FetchParamsWithCacheData<T>,
	): boolean {
	 ...
	 * @param {FetchParamsWithCacheData<T>} params - Fetch parameters with cache data.
	 */
	private validateFetchParams<T extends ApiObj>(
		params: FetchParamsWithCacheData<T>,
	): void {
		const { cacheKey, maxId, maxIdForFetch, minIdForFetch, moar, skipCache } =
			params;
		...
		if (maxIdForFetch && minIdForFetch) {
			logger.logAndThrowError(
				`maxIdForFetch and minIdForFetch can't be used at same time!`,
				params,
			);
		}
		...
		if (cacheKey != CacheKey.HASHTAG_TOOTS) {
			const paramsToLog = removeKeys(
				params,
				PARAMS_TO_NOT_LOG,
				PARAMS_TO_NOT_LOG_IF_FALSE,
			);

			if (paramsToLog.minIdForFetch) {
				logger.debug(
					`Incremental fetch from API to update stale cache:`,
					paramsToLog,
				);
