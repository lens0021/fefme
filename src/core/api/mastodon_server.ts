...
const loggers: Record<FediverseCacheKey, Logger> =
	Logger.buildEnumLoggers(FediverseCacheKey);
const mutexes = simpleCacheKeyDict(
	() => new Mutex(),
...
export default class MastodonServer {
	...
	private async fetch<T>(endpoint: string, limit?: number): Promise<T> {
		...
		if (json.status === 200 && json.data) {
			this.logger.deep(
				`(${endpoint}) fetch response ${ageString(startedAt)}:`,
				json.data,
			);
		...
		try {
			...
			if (!list) {
				logger.logAndThrowError(
					`No ${label} found! list: ${JSON.stringify(list)}`,
				);
	...
	static async getMastodonInstances(): Promise<MastodonInstances> {
		...
		const releaseMutex = await lockExecution(
			mutexes[cacheKey],
			getLogger(cacheKey, "getMastodonInstances"),
		);
	...
	private static async fetchMastodonInstances(): Promise<MastodonInstances> {
		...
		const followedUserDomainCounts = countValues(
			follows,
			(account) => account.homeserver,
		);
		...
		const servers = Object.entries(serverDict)
			.filter(([_k, v]) => !!v)
			.reduce((serverDict, [domain, _instance]) => {
				...
				instance.followedPctOfMAU = instance.MAU
					? domainAccountsFollowed / instance.MAU
					: 0;
		...
		logger.log(
			`Fetched ${numServers} Instances ${ageString(startedAt)}:`,
			servers,
		);
		return servers;
	}
		...
		try {
			...
			if (!objs?.length) {
				...
				logger.debugWithTraceObjs(
					`fetched ${objs.length} objs ${ageString(startedAt)}`,
					objs,
				);
	...
	private static async callForServers<T>(
		...
		return await zipPromiseCalls<T>(
			domains,
			async (domain) => fxn(new MastodonServer(domain)),
			buildLogger(),
		);
	}
