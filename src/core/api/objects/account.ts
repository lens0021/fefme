...
const ACCOUNT_CREATION_FMT: Intl.DateTimeFormatOptions = {
	year: "numeric",
	month: "short",
	day: "numeric",
};
...
export default class Account implements AccountObj {
	...
	get isLocal(): boolean {
		return MastoApi.instance.isLocalUrl(this.url);
	}
	get localServerUrl(): string {
		return MastoApi.instance.accountUrl(this);
	}
	...
	private get buildWebfingerURI(): string {
		return (
			this.acct.includes("@") ? this.acct : `${this.acct}@${this.homeserver}`
		).toLowerCase();
	}
	...
	displayNameFullHTML(fontSize: number = DEFAULT_FONT_SIZE): string {
		return (
			this.displayNameWithEmojis(fontSize) + encode(` (@${this.webfingerURI})`)
		);
	}
	...
	displayNameWithEmojis(fontSize: number = DEFAULT_FONT_SIZE): string {
		return replaceEmojiShortcodesWithImgTags(
			this.displayName,
			this.emojis || [],
			fontSize,
		);
