...
export default class CountedList<T extends NamedTootCount> {
	...
	public set objs(objs: T[]) {
		...
		this.nameDict = this.objs.reduce(
			(objNames, obj) => {
	...
	addObjs(objs: T[]): void {
		...
		const addableObjs = objs
			.filter((obj) => !this.nameDict[obj.name])
			.map((obj) => this.completeObjProperties(obj));
		this.objs = [...this.objs, ...addableObjs];
		this.logger.trace(
			`addObjs() added ${addableObjs.length} of ${objs.length} incoming objs to initial ${numObjsBefore}:`,
			addableObjs,
		);
	}
	...
	populateByCountingProps<U>(objs: U[], propExtractor: (obj: U) => T): void {
		this.logger.deep(
			`populateByCountingProps() - Counting properties in ${objs.length} objects...`,
		);
	...
	removeKeywords(keywords: string[]): void {
		keywords = keywords.map((k) =>
			(k.startsWith("#") ? k.slice(1) : k).toLowerCase().trim(),
		);
		const validObjs = this.objs.filter((tag) => !keywords.includes(tag.name));
		this.logger.logArrayReduction(
			this.objs,
			validObjs,
			"Tag",
			`matching keywords`,
	...
	topObjs(maxObjs?: number): T[] {
		...
		this.logger.trace(
			`topObjs() sorted ${this.objs.length} first 100 objs:`,
			sortedObjs.slice(0, 100),
		);
