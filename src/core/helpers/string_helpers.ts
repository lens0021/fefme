...
export const at = (str: string): string =>
	str.startsWith("@") ? str : `@${str}`;
/** @example "foo" => "<foo>" */
export const arrowed = (str: string): string =>
	str.startsWith("<") ? str : `<${str}>`;
/** @example "string" => "[string]" */
export const bracketed = (str: string): string =>
	str.startsWith("[") ? str : `[${str}]`;
/** @example 'string' => '"string"' */
export const quoted = (str: string | null): string =>
	isNil(str) ? NULL : `"${str}"`;
...
export const collapseWhitespace = (str: string) =>
	str.replace(WHITESPACE_REGEX, " ").replace(/\s,/g, ",").trim();
/** @example ("ó" => "o", "é" => "e", etc.) */
...
export const removeMentions = (str: string) =>
	str.replace(ACCOUNT_MENTION_REGEX, " ");
...
export const byteString = (numBytes: number): string => {
	...
	if (numBytes < MEGABYTE)
		return `${(numBytes / KILOBYTE).toFixed(1)} kilobytes`;
...
export function determineMediaCategory(
	...
	Object.entries(MEDIA_FILE_EXTENSIONS).forEach(
 ...
 * @returns {string}
 */
export function optionalSuffix<T>(
	obj: T,
	toSuffix?: ((obj: T) => string) | string,
	noSpace?: boolean,
): string {
	if (isNil(obj)) return "";
	toSuffix ??= (o: T) => `${o}`;
