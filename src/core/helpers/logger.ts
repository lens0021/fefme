...
export class Logger {
	...
	constructor(name: string, ...args: LoggerArg[]) {
		this.prefixes = [
			name,
			...(args.filter((arg) => typeof arg == "string") as string[]),
		];
		this.logPrefix = this.prefixes
			.map((str, i) => (PREFIXERS[i] ? PREFIXERS[i](str) : str))
			.join(" ");
	...
	deep = (msg: string, ...args: unknown[]) => {
		isDeepDebug && this.debug(msg, ...args);
	};
	/** Logs a warning message with a warning colored prefix (not a real warning level). */
	warnWithoutTrace = (msg: string, ...args: unknown[]) =>
		console.log(`%cWarning: ${msg}`, "color: orange;", args);
	...
	logAndThrowError(msg: string, ...args: unknown[]): never {
		...
		throw errorArgs.error
			? new Error(msg, { cause: errorArgs.error })
			: new Error(msg);
	}
	...
	logArrayReduction<T>(
		...
		this.trace(
			`Removed ${numRemoved} ${objType}s${optionalSuffix(reason)} leaving ${after.length}`,
		);
	}
	...
	logTelemetry(msg: string, startedAt?: Date, ...args: unknown[]): void {
		...
		if (startedAt) {
			...
			this.warn(
				`logTelemetry() called without startedAt, no elapsed time will be logged`,
			);
		}
	...
	private errorStr(...args: unknown[]): string {
		const errorArgs = this.findErrorArg(args);
		const stringArgs = errorArgs.args.map((arg) =>
		...
		if (errorArgs.error) {
			...
			if (!stringArg)
				this.warn(
					`errorStr() called with no string or error args, returning empty string`,
				);
			return stringArg || "";
		}
	}
	...
	private findErrorArg(args: unknown[]): ErrorArgs {
		...
		if (errorArgs.length > 0) {
			if (errorArgs.length > 1) {
				this.warn(
					`findErrorArg() called with multiple Error args, only using the first one:`,
					errorArgs,
				);
			}
	 ...
	 */
	static buildEnumLoggers<E extends string, T extends Record<string, E>>(
		strEnum: T,
	): Record<E, Logger> {
		return Object.values(strEnum).reduce(
			(loggers, value) => {
