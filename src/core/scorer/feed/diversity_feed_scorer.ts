		...
		sortedToots.forEach((toot) => {
			...
			(toot.realToot.trendingTags ?? []).forEach((tag) => {
				...
				penalizedTag.penaltyIncrement =
					penalizedTag.numAccounts / penalizedTag.numToots!;
				penalizedTag.numToPenalize =
					penalizedTag.numToots! -
					config.scoring.diversityScorerMinTrendingTagTootsForPenalty;
			});
		});
	...
	async _score(toot: Toot) {
		...
		if (score < 0) {
			if (score > -0.2) {
				...
				console.warn(
					`Negative diversity score of ${score.toFixed(2)} for toot: ${toot.description}:`,
					toot,
				);
			}
		...
		return toot.reblog
			? score * config.scoring.diversityScorerRetootMultiplier
			: score;
	}
