...
export default function FindFollowers(): React.ReactElement {
	...
	useEffect(() => {
		...
		(
			Promise.resolve(api.v2.suggestions.list()) as Promise<
	...
	return (
		<Accordion style={accordionStyle}>
			<Accordion.Item eventKey="findfollowers">
				<Accordion.Header>
					<p style={titleStyle}>Find Followers</p>
				</Accordion.Header>
				...
				<Accordion.Body onEnter={() => setOpen(true)}>
					<Row className="g-4 m-3">
						...
						{suggestions
							.filter(
								(suggestion: mastodon.v1.Suggestion) =>
									suggestion.source === "past_interactions",
							)
