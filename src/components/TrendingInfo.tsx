...
export default function TrendingInfo(): JSX.Element {
	...
	useEffect(() => {
		...
		if (isLoading) {
			bustLogger.trace(
				`Trending data is loading, not busting cache yet. Num trending links: ${numTrendingLinks}`,
			);
			return;
		}
		...
		const bustCacheIfNoTrendingData = async () => {
			bustLogger.trace(
				`bustCacheIfNoTrendingData() invoked, about to sleep for ${SLEEP_SECONDS} seconds...`,
			);
			await sleep(SLEEP_SECONDS * 1000); // Wait for SLEEP_SECONDS to see if trending data is loaded
			bustLogger.trace(
				`Trending data not loaded after ${SLEEP_SECONDS} seconds, calling refreshTrendingData()`,
			);
		...
		return (
			<TrendingSection
				...
				linkRenderer={{
					...
					onClick: (domain: string, e) =>
						followUri(sanitizeServerUrl(domain, true), e),
				}}
	...
	const linksSection = useMemo(() => {
		return (
			<TrendingSection
				linkRenderer={{
					...trendingObjLinkRenderer,
					linkLabel: (link: TrendingLink) =>
						prefixedHtml(link.title, extractDomain(link.url)),
				}}
	...
	return (
		<TopLevelAccordion bodyStyle={noPadding} title="What's Trending">
			<div style={accordionSubheader}>
				<p style={{}}>
					Trending data was scraped from{" "}
					{Object.keys(trendingData.servers).length} Mastodon servers.
				</p>
			</div>
...
const simpleTagRenderer: LinkRenderer = {
	...baseLinkRenderer,
	infoTxt: (tag: TagWithUsageCounts) =>
		`${tag.numToots?.toLocaleString()} toots`,
	linkLabel: tagNameMapper,
};
