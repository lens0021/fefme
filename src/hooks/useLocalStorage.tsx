...
export function useServerStorage(): [string, (value: string) => string] {
	const [server, setRawServer] = useLocalStorage<string>(
		SERVER,
	...
	const setServer = (server: string): string => {
		...
		logger.trace(
			`useServerStorage() setting server to sanitized "${sanitizedServer}"`,
		);
 ...
 */
export const useServerUserStorage = (): ServerUserState =>
	useLocalStorage<Record<string, ServerUser>>(SERVER_USERS, {});
...
): [App | null, (app: App) => void] {
	const setApp = (app: App) =>
		setServerProperties(serverUserState, app, undefined);
...
export function useUserStorage(
	...
	const setUser = (user: User) =>
		setServerProperties(serverUserState, undefined, user);
...
function getServer(): string {
	...
	if (!server) {
		logger.trace(
			`No server found in localStorage, using default: "${config.app.defaultServer}"`,
		);
		window.localStorage.setItem(
			SERVER,
			JSON.stringify(config.app.defaultServer),
		);
